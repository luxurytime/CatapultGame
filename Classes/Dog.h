...
public:
	Dog()
	{
		auto texture = Director::getInstance()->getTextureCache()->addImage("knight_0b.png");
		auto frame0 = SpriteFrame::createWithTexture(texture, CC_RECT_PIXELS_TO_POINTS(Rect(0, 0, 79.25, 100)));
		...
		walk.reserve(5);
		for (int i = 0; i < 4; i++) {
			walk.pushBack(SpriteFrame::createWithTexture(texture, CC_RECT_PIXELS_TO_POINTS(Rect(79.25 * i, 0, 79.25, 100))));
		}
